jQuery(document).ready(function($){
	...
	function triggerAnimation(){
		if(MQ == 'desktop') {
			...
			$('.cd-section-curtain').find('.cd-block').removeAttr('style').find('.cd-half-block').removeAttr('style');
		}
	...
	function animateSection () {
		...
		$('.cd-section-curtain').each(function(){
	...
	function nextSection() {
		if (!animating) {
			if ($('.cd-section-curtain.is-visible').next().length > 0) smoothScroll($('.cd-section-curtain.is-visible').next());
		}
	}

	function prevSection() {
		if (!animating) {
			var prevSection = $('.cd-section-curtain.is-visible');
			if(prevSection.length > 0 && $(window).scrollTop() != prevSection.offset().top) {
				smoothScroll(prevSection);
			} else if(prevSection.prev().length > 0 && $(window).scrollTop() == prevSection.offset().top) {
				smoothScroll(prevSection.prev('.cd-section-curtain'));
			}
		}
	}
